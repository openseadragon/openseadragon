...
(function() {
    ...
    QUnit.test('Test data generators produce valid data', function (test) {
        ...
        (async function() {
            ...
            // Test image generator (async because image decoding must finish)
            const image = await Converter._testDataGenerators['image'](32);
            test.ok(image, "image generator returns data");
            test.equal(image.width, 32, "image has correct width");
    ...
    QUnit.test('benchmark handles async conversions', function (test) {
        ...
        (async function() {
            ...
            test.ok(result.measurements[32].median > 0, "Measured time includes async delay");
    ...
    // ----------
    QUnit.test('benchmark rejects when generator or conversion fails', function(test) {
        const done = test.async();

        (async function() {
            try {
                await Converter.benchmark('context2d', function() {
            ...
            Converter.registerTestDataGenerator('__TEST__rejectingGenerator', function() {
                return $.Promise.reject(new Error('generator failure'));
            });

            try {
                await Converter.benchmark('__TEST__rejectingGenerator', function(tile, value) {
                    return value;
                }, {
                    sizes: [32],
                    iterations: 1,
